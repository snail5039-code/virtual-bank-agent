...
#
#   START → extract → check ─┬─ 바꿀 수 있음 → authenticate → propose → approve → interpret ─┬─ 승인      → execute ─┐
#              ↑             │                                            ↑                 ├─ 거절·취소 → reject  ─┤
#              │             └─ 계좌 없음·바꿀 수 없음 → fail → END       └──── 모름 ───────┤                       │
#              └───────────────────────────────────────────────────────────── 수정 ───────┘                       │
#                                                                                                                  │
...
from agents.setting.nodes import (route_after_authenticate, route_after_check, route_after_interpret,
                                  setting_check_node, setting_execute_node, setting_extract_node,
...
builder.add_edge("setting_extract", "setting_check")
